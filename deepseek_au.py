...
# 初始化OKX交易所 - 修改为支持黄金的配置
exchange = ccxt.okx({
    'options': {
        'defaultType': 'spot',  # 对于黄金现货交易
    },
...
TRADE_CONFIG = {
    ...
    'position_management': {
        ...
        'base_usdt_amount': 100,  # USD投入下单基数
...
def setup_exchange():
    ...
    try:
        ...
        gold_market = markets[TRADE_CONFIG['symbol']]

        # 获取合约乘数
        contract_size = float(gold_market['contractSize']) if 'contractSize' in gold_market else 1.0
        print(f"✅ 合约规格: 1张 = {contract_size} XAU")

        # 存储合约规格到全局配置
        TRADE_CONFIG['contract_size'] = contract_size
        TRADE_CONFIG['min_amount'] = gold_market['limits']['amount']['min']

        print(f"📏 最小交易量: {TRADE_CONFIG['min_amount']} 张")

        # 先检查现有持仓
        print("🔍 检查现有持仓模式...")
        positions = exchange.fetch_positions([TRADE_CONFIG['symbol']]) if hasattr(exchange, 'fetch_positions') else []
        ...
        for pos in positions:
            if pos['symbol'] == TRADE_CONFIG['symbol']:
                contracts = float(pos.get('contracts', 0)) if pos.get('contracts') else 0
        ...
        try:
            ...
            print(f"⚠️ 设置杠杆失败 (可能不支持): {e}")

        # 5. 验证设置
        print("🔍 验证账户设置...")
        balance = exchange.fetch_balance()
        usd_balance = balance.get('USD', {}).get('free', balance.get('USDT', {}).get('free', 0))
        print(f"💰 当前USD余额: {usd_balance:.2f}")
...
def calculate_intelligent_position(signal_data, price_data, current_position):
    ...
    try:
        ...
        # 基础USD投入
        base_usd = config['base_usdt_amount']
        print(f"💰 可用USD余额: {usd_balance:.2f}, 下单基数{base_usd}")
        ...
        # 计算建议投入USD金额
        suggested_usd = base_usd * confidence_multiplier * trend_multiplier * rsi_multiplier

        # 风险管理：不超过总资金的指定比例 - 删除重复定义
        max_usd = usd_balance * config['max_position_ratio']
        final_usd = min(suggested_usd, max_usd)

        # 正确的合约张数计算！
        # 公式：合约张数 = (投入USD) / (当前价格 * 合约乘数)
        contract_size = (final_usd) / (price_data['price'] * TRADE_CONFIG['contract_size'])

        print(f"📊 仓位计算详情:")
        print(f"   - 基础USD: {base_usd}")
        print(f"   - 信心倍数: {confidence_multiplier}")
        print(f"   - 趋势倍数: {trend_multiplier}")
        print(f"   - RSI倍数: {rsi_multiplier}")
        print(f"   - 建议USD: {suggested_usd:.2f}")
        print(f"   - 最终USD: {final_usd:.2f}")
        ...
        print(f"🎯 最终仓位: {final_usd:.2f} USD → {contract_size:.2f} 张合约")
        ...
        # 紧急备用计算
        base_usd = config['base_usdt_amount']
        contract_size = (base_usd * TRADE_CONFIG['leverage']) / (
                    price_data['price'] * TRADE_CONFIG.get('contract_size', 1.0))
...
def get_sentiment_indicators():
    """获取情绪指标 - 简洁版本 - 注意：黄金情绪指标可能需要不同的API"""
    try:
        # 注意：这里的API是针对加密货币的，黄金可能需要其他数据源
        # 这里留作占位符，实际使用时需要替换为黄金情绪数据源
        print("💡 黄金情绪数据暂未接入，使用默认值")
        return None
        
        # 以下代码是原始的加密货币情绪API，可能不适用于黄金
...
def analyze_with_deepseek(price_data):
    ...
    if sentiment_data:
        ...
        sentiment_text = "【市场情绪】数据暂不可用（黄金情绪指标需另行接入）"

    # 添加当前持仓信息
    current_pos = get_current_position()
    position_text = "无持仓" if not current_pos else f"{current_pos['side']}仓, 数量: {current_pos['size']}, 盈亏: {current_pos['unrealized_pnl']:.2f}USD"
    pnl_text = f", 持仓盈亏: {current_pos['unrealized_pnl']:.2f} USD" if current_pos else ""

    prompt = f"""
    你是一个专业的黄金交易分析师。请基于以下XAU/USD {TRADE_CONFIG['timeframe']}周期数据进行分析：
    ...
    - 本K线成交量: {price_data['volume']:.2f} XAU
    ...
    try:
        response = deepseek_client.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": f"您是一位专业的黄金交易员，专注于{TRADE_CONFIG['timeframe']}周期趋势分析。请结合K线形态和技术指标做出判断，并严格遵循JSON格式要求。"},
